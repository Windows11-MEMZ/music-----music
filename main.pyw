...
def openfile(index = [1]):
  ...
  filenames =  filedialog.askopenfilenames(title='打开文件',filetypes=[("mp3文件","*.mp3"),("WMA文件","*.wma"),("WAV文件","*.wav")])
  if filenames:
...
root =Tk()
root.title('Localage Music')
wmp = Dispatch("WMPlayer.OCX")

canvas = Canvas(root,width =150,height = 100,bg = "white")
img = PhotoImage(file = '111.gif')
...
var_volume = IntVar()
vioce_lab = LabelFrame(root,text = "音量")
...
vio_plus = Button(vioce_lab,width =8,text = "增加",command =Volume_add)
vio_plus.grid(row =9,column =0,sticky = "w")
vio_minus = Button(vioce_lab,width =8,text ="减少",command = Volume_minus)
